...
public interface UserRepository extends JpaRepository<CustomUser, Long> {
    ...
    @Query("SELECT CASE WHEN COUNT(u) > 0 THEN true ELSE false END FROM CustomUser u "
            + "WHERE u.login = :login")
    boolean existsByLogin(@Param("login") String login);

    @Query("SELECT CASE WHEN COUNT(u) > 0 THEN true ELSE false END FROM CustomUser u "
            + "INNER JOIN u.followers c "
            + "WHERE (u.id = :id1) AND (c.id = :id2)")
    boolean isFollowing(@Param("id1") long id1, @Param("id2") long id2);

    @Query("SELECT COUNT(u) FROM CustomUser u "
            + "INNER JOIN u.followers c "
            + "WHERE u.id = :id1")
    ...
    @Query("SELECT c FROM CustomUser u "
            + "INNER JOIN u.followers c "
            + "WHERE (u.id = :id1)")
    ...
    @Query("SELECT u FROM CustomUser u "
            + "INNER JOIN u.followers c "
            + "WHERE c.id IN (SELECT u.id FROM CustomUser u "
            + "INNER JOIN u.followers c "
            + "WHERE c.id = :id1)")
