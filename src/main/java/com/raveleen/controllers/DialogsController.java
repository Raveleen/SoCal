...
public class DialogsController {
    ...
    public String redirectOrCreateDialog(@PathVariable("user-id") long id) {
        ...
        if (dialogService.isDialogExists(customUser.getId(), customUserSecond.getId())) {
            System.out.println("dialog " + customUser.getId() + customUserSecond.getId() + "exists(true)");
            Dialog temp = dialogService.getDialog(customUser.getId(), customUserSecond.getId());
            return "redirect:/dialog/" + temp.getId();
        } else {
            System.out.println("dialog " + customUser.getId() + customUserSecond.getId() + "don't exists(false)");
    ...
    public String userAccount(Model model, @PathVariable("dialog-id") long id) {
        ...
        if (us1.getId() == dbUser.getId()) {
            model.addAttribute("dialogUser", us2);
            model.addAttribute("dialogUserFollowers", userService.getNumberOfFollowers(us2.getId()));
            model.addAttribute("dialogUserFollowing", userService.getNumberOfFollowings(us2.getId()));
        } else {
            model.addAttribute("dialogUser", us1);
            model.addAttribute("dialogUserFollowers", userService.getNumberOfFollowers(us1.getId()));
            model.addAttribute("dialogUserFollowing", userService.getNumberOfFollowings(us1.getId()));
        }
    ...
    public String[] getDialogsList(@PathVariable("from") int from) {
        ...
        for (Dialog temp : dialogs) {
            ...
            if (us1.getId() == customUser.getId()) {
                response[i] = utilsService.createFragmentDialog(temp, customUser, us2);
            } else {
                response[i] = utilsService.createFragmentDialog(temp, customUser, us1);
            }
    ...
    @ResponseBody
    public String[] getMessagesList(@PathVariable("dialog-id") long dialogId,@PathVariable("from") int from) {
        User user = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        String login = user.getUsername();
        CustomUser customUser = userService.getUserByLogin(login);
        ...
        for (Message message : messages) {
            ...
            response[i] = utilsService.createFragmentMessage(message, customUser);
            i++;
        }
    ...
    @ResponseBody
    public String[] getMessagesList(@PathVariable("dialog-id") long dialogId,@PathVariable("time") long time) {
        User user = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        String login = user.getUsername();
        CustomUser customUser = userService.getUserByLogin(login);
    ...
    @ResponseBody
    public String createMessage(@RequestParam("message-text") String text, @PathVariable("dialog-id") long dialogId) throws IOException {
        User user = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        String login = user.getUsername();
        CustomUser customUser = userService.getUserByLogin(login);
